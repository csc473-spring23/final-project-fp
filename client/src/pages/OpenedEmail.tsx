...
const OpenedEmail = () => {
    ...
    const submitHandle = () => {
        console.log(reply);
        axios.post("http://localhost:3001/emails", {emailText: reply, 
                                                    senderEmail: localStorage.getItem("email"),
                                                    receiEmail: email.senderEmail,
                                                    subject: `re to: ${email.subject}`,
                                                    replyToId: email.id,}).then((resp) => {
  ...
  return (
    <Container style= {{width: "100%"}} className = "mb-0 ms-0 me-0 d-flex flex-row">
    <Card style= {{width: "50%"}} className='h-200'>
        <Card.Body>
    ...
    {replyEmails.map((value, key) => {
            return (
                <ListGroup.Item key = {value.id} className='d-flex flex-row me-0' style= {{width: "300%"}}>
